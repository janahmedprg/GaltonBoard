...
while(True):
    ...
    csvName = 'gdata_'+'eta_'+str(eta)+'time_'+str(timeCap-increment)+'_particle'+str(particle_idx)+'_grav'+str(gravity)+'.csv'
    rf = open(csvName,'r')
    reader = csv.reader(rf)

    row = next(reader)
    gdata.append(float(row[0]))
    gdata.append(float(row[1]))
    gdata.append(float(row[2]))
    gdata.append(float(row[3]))
    gdata.append(float(row[4]))
    gdata.append(float(row[5]))
    gdata.append(float(row[6]))
    gdata.append(int(row[7]))
    gdata.append(bool(row[8]))
    gdata.append(float(row[9]))

    rf.close()

    print(gdata[0],gdata[1],gdata[2],gdata[3],gdata[4],gdata[5],gdata[6],gdata[7],gdata[8],gdata[9])

    csvName = 'gdata_'+'eta_'+str(eta)+'time_'+str(timeCap)+'_particle'+str(particle_idx)+'_grav'+str(gravity)+'.csv'
    ...
    vY=gdata[5]
    vS=gdata[6]
    isTorus=gdata[8] # Don't change unless we are starting on the disperses
    while time<timeCap:
